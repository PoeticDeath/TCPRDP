        ...
        def keys(self):
            while True:
                ...
                match data[0:2]:
                    ...
                    case b"ME":
                        mouse.position = (int.from_bytes(key[0:2], "big"), int.from_bytes(key[2:4], "big"))
                    case b"MM":
                        mouse.move(int.from_bytes(key[0:2], "big", signed=True), int.from_bytes(key[2:4], "big", signed=True))
                    case b"MC":
                        mouse.click(eval(key[:-1]), key[-1])
                    case b"MS":
                        mouse.scroll(int.from_bytes(key[0:1], "big", signed=True), int.from_bytes(key[1:2], "big", signed=True))
...
def client():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        def on_release(key):
            ...
            sock.sendall(str(key).encode())
        keylistener = pynput.keyboard.Listener(on_press=on_press, on_release=on_release, suppress=True)
        keylistener.start()
        def on_click(x, y, button, pressed):
            sock.sendall(b"MC"+len(str(button).encode()+pressed.to_bytes(1, "big")).to_bytes(4, "big"))
            sock.sendall(str(button).encode()+pressed.to_bytes(1, "big"))
        def on_scroll(x, y, dx, dy):
            sock.sendall(b"MS"+len(dx.to_bytes(1, "big", signed=True)+dy.to_bytes(1, "big", signed=True)).to_bytes(4, "big"))
            sock.sendall(dx.to_bytes(1, "big", signed=True)+dy.to_bytes(1, "big", signed=True))
        ...
        sock.sendall(oldmousepos[0].to_bytes(2, "big")+oldmousepos[1].to_bytes(2, "big"))
        try:
            while True:
                frame = b""
                data = sock.recv(5)
                ...
                if oldmousepos != mouse.position:
                    ...
                    sock.sendall(x.to_bytes(2, "big", signed=True)+y.to_bytes(2, "big", signed=True))
                press = set()
        except KeyboardInterrupt:
            pass
